...
const generateListQueue = () => {
  ...
  for (let i = 0; i < queue.MAX_SIZE - queue.display().length; i++) {
    ...
    queueElement.innerHTML = '&nbsp;'; // to get that 'empty' yet size adequate display

    queueElement.classList.add('inactive');
    queueUL.insertBefore(queueElement, queueUL.firstChild);
  }
  queue.display().forEach((elem) => {
    const queueElement = document.createElement('li');
    queueElement.classList.add('active');
    queueElement.textContent = elem;
    queueUL.insertBefore(queueElement, queueUL.firstChild);
  });
};
